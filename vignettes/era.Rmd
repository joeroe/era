...
```{r yr-transform-precise}
yr(500000, "BCE") %>% 
  yr_transform(era("ka"))
```

Often, this precision is not necessary.
For example, when converting years on a geologic time scale between calendar- and present-based eras, the `r this_year - 1950` year difference between the formal definition of "Present" and the actual present is rarely significant.
Use the `precision` argument of `yr_transform` to get prettier results:

```{r yr-transform-pretty}
yr(10000, "BP") %>% 
...
## Arithmetic with years {#arithmetic}

The `yr` class ensures type- and size-stable computations with years.
For example, you can perform arithmetic:
...
This means that it is possible to combine yrs with differently named but functionally equivalent eras, for example `era("BP")` and `era("cal BP")`, although doing so will cause a warning about the loss of information:

```{r eg-flexible-compare}
era("BP") == era("BC")
era("BP") == era("cal BP")
